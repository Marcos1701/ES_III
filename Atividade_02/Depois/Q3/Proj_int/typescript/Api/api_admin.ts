...
async function get_email(token: string): Promise<string> {
    const retorno = await client.query(`SELECT email FROM usuario WHERE token = $1`, [token]).then((result: any) => {
        return result;
    });
...
async function get_usuarios(req: Request, res: Response) {
    ...
    const id_usuario = await client.query(`SELECT GET_ID_USUARIO($1)`, [email]).then((result: { rows: { get_id_usuario: any; }[]; }) => {
    ...
    const usuarios: any[] = await client.query(`SELECT * FROM GET_USUARIOS($1)`, [id_usuario]).then((result: { rows: any; }) => {
        return result.rows
    }).catch((err: any) => {
        console.log(err);
    ...
    const admins: any[] = await client.query(`SELECT * FROM GET_ADMINS($1)`, [id_usuario]).then((result: { rows: any; }) => {
        return result.rows;
    }).catch((err: any) => {
        console.log(err);
...
async function adicionar_admin(req: Request, res: Response) {
    ...
    const id_admin = await client.query(`SELECT GET_ID_USUARIO($1)`, [email]).then((result: { rows: { get_id_usuario: any; }[]; }) => {
        return result.rows[0].get_id_usuario;
    }).catch((err: any) => {
        console.log(err);
        return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
    ...
    client.query(`SELECT ADICIONAR_ADM($1, $2)`, [id_admin, id_usuario], (err: any, result: any) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
        }
...
async function confere_admin(req: Request, res: Response) {
    try {
        ...
        const retorno = await client.query(`SELECT * FROM CONSULTAR_ADM($1)`, [id_admin]).then((result: { rows: any; }) => {
            return result.rows;
        })
...
async function get_tarefas_usuario(req: Request, res: Response) {
    ...
    client.query(`SELECT GET_TAREFAS_USUARIO($1, $2)`, [id_admin, id_usuario], (err: any, result: { rows: { get_tarefas_usuario: any; }[]; }) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
        }
...
async function get_all_tarefas(req: Request, res: Response) {
    ...
    client.query(`SELECT * FROM GET_STATUS_ALL_TAREFAS($1)`, [id_admin], (err: any, result: { rows: any; }) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
        }
...
async function get_tarefas_pendentes(req: Request, res: Response) {
    ...
    client.query(`SELECT GET_TAREFAS_PENDENTES($1)`, [id_usuario], (err: any, result: { rows: { get_tarefas_pendentes: any; }[]; }) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
        }
...
async function get_tarefas_concluidas(req: Request, res: Response) {
    ...
    client.query(`SELECT GET_TAREFAS_CONCLUIDAS($1)`, [id_usuario], (err: any, result: { rows: { get_tarefas_concluidas: any; }[]; }) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
        }
...
async function get_tarefas_atrasadas(req: Request, res: Response) {
    ...
    client.query(`SELECT GET_TAREFAS_ATRASADAS($1)`, [id_usuario], (err: any, result: { rows: { get_tarefas_atrasadas: any; }[]; }) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
        }
