...
async function adicionar_tarefa(req: Request, res: Response) {
    ...
    if (data === undefined || data === null) {
        client.query(`SELECT ADICIONAR_TAREFA($1, $2, $3, $4, $5)`, [id, titulo, descricao, id_usuario, prioridade], (err: any, result: any) => {
            if (err) {
                console.log(err);
                return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
            }
        ...
        client.query(`SELECT ADICIONAR_TAREFA($1, $2, $3, $4, $5, $6)`, [id, titulo, descricao, id_usuario, prioridade, new Date(data)], (err: any, result: any) => {
            if (err) {
                console.log(err);
                return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
            }
...
async function editar_tarefa(req: Request, res: Response) {
    ...
    if (data === undefined || data === null) {
        client.query(`SELECT EDITAR_TAREFA($1, $2, $3, $4, $5, NULL)`, [id_usuario, id, titulo, descricao, prioridade], (err: any, result: any) => {
            if (err) {
                console.log(err);
                return res.status(500).json({ erro: "Erro ao acessar o banco de dados" });
            }
        ...
        client.query(`SELECT EDITAR_TAREFA($1, $2, $3, $4, $5, $6)`, [id_usuario, id, titulo, descricao, prioridade, data], (err: { message: any; }, result: any) => {
            if (err) {
                return res.status(500).json({ erro: err.message });
            }
            return res.status(200).json({ id: id });
...
async function excluir_tarefa(req: Request, res: Response) {
    ...
    client.query(`SELECT DELETAR_TAREFA($1, $2)`, [id, id_usuario], (err: { message: any; }, result: any) => {
        if (err) {
            return res.status(500).json({ erro: err.message });
        }
...
async function get_tarefas(req: Request, res: Response) {
    ...
    const tarefas = await client.query(`SELECT * FROM TAREFA WHERE ID_USUARIO = $1`, [id_usuario]).then((result: { rows: any; }) => {
        return result.rows;
    }).catch((err: { message: any; }) => {
        return res.status(500).json({ erro: err.message });
    }) as any[];
...
async function concluir_tarefa(req: Request, res: Response) {
    ...
    client.query(`SELECT CONCLUIR_TAREFA($1, $2)`, [id, id_usuario], (err: { message: any; }, result: any) => {
        if (err) {
            return res.status(500).json({ erro: err.message });
        }
...
async function desconcluir_tarefa(req: Request, res: Response) {
    ...
    client.query(`SELECT DESCONCLUIR_TAREFA($1, $2)`, [id, id_usuario], (err: { message: any; }, result: any) => {
        if (err) {
            return res.status(500).json({ erro: err.message });
        }
        return res.sendStatus(200);
...
async function get_tarefa(req: Request, res: Response) {
    ...
    const tarefa: any = await client.query(`SELECT GET_TAREFA($1, $2)`, [id, id_usuario], (err: any, result: { rows: { get_tarefa: any; }[]; }) => {
        if (err) {
            console.log(err);
...
setInterval(() => {
    ...
    client.query(`SELECT ATUALIZAR_TAREFAS()`, (err: any, result: any) => {
        if (err) {
            console.log(err);
        }
    })
