...
async function get_name(token: string): Promise<string | null> {
    const name = await client.query(`SELECT nome FROM usuario WHERE token = '${token}'`).then((result: { rows: { nome: any; }[]; }) => {
...
async function get_senha(token: string): Promise<string | null> {
    const senha = await client.query(`SELECT senha FROM usuario WHERE token = '${token}'`).then((result: { rows: { senha: string; }[]; }) => {
...
async function editar_usuario(req: Request, res: Response) {
    ...
    client.query("SELECT EDITAR_USUARIO($1, $2, $3, $4, $5)", [token, novo_token ? novo_token : null, nome, novo_email ? email : null, nova_senha ? senha : null], (err: { message: any; }, result: any) => {
        if (err) {
...
async function excluir_usuario(req: Request, res: Response) {
    ...
    client.query("SELECT EXCLUIR_USUARIO($1)", [token], (err: any, result: any) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ error: "Erro ao acessar o banco de dados" });
        }
...
async function get_data(req: Request, res: Response) {
    ...
    client.query("SELECT * FROM GET_DATA($1)", [token], (err: any, result: { rows: any[]; }) => {
        if (err) {
            console.log(err);
            return res.status(500).json({ error: "Erro ao acessar o banco de dados" });
        }
