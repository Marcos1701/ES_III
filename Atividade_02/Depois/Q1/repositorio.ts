...
class lista_duplamente_encadeada<T> {
    ...
    // 1° adição de comentários em alguns métodos
    /***
    ...
    push(Valor: T): void {
        ...
        // 2° Remoção de elses desnecessários
        if (this._fim) {
    ...
    pop(): T {
        ...
        // 3° Alteração de let para const em algumas variáveis
    ...
    pop_valor(valor: T): void {
        ...
        if (no_aux == null) { throw new Valor_inexistente("Ops, não há nenhum valor para ser removido!!") }
    ...
    front(): T {
        if (this._inicio == null) { throw new Valor_inexistente("Valor inexistente") }
...
class repo_usuarios implements Repositorio_encadeado<Usuario>{
    // 5° Alterando o atributo lista_usuarios para privado
    private lista_usuarios: lista_duplamente_encadeada<Usuario> = new lista_duplamente_encadeada<Usuario>()
    ...
    excluir(id: number): void {
        try {
            const usuario: Usuario = this.consultar(id).Valor
    ...
    autenticar(login: string, senha: string): Usuario {
        const usuario: Usuario = this.consultar_conta(login).Valor
        usuario.autenticar(login, senha);
        return usuario
    }

    inserir_nova_rede(rede: RedeSocial, id_usuario: number): void {
        const no: No<Usuario> = this.consultar(id_usuario)
...
class repo_redes_sociais implements Repositorio_encadeado<RedeSocial> {
    ...
    exibir_redes(): void {
        if (this._redes_sociais.inicio == null) {
            throw new rede_social_inexistente("Erro, não há redes sociais cadastradas!!")
        }
    ...
    consultar_usuarios_cadastrados_em_det_rede_social(id_rede: number, id_usuario_principal: number): number[] {
        ...
        for (let usuario of usuarios_da_rede) {
            if (usuario.id != id_usuario_principal) {
                console.log(`ID: ${usuario.id}`)
                console.log(`Nome: ${usuario.nome}\n`)
                ids_usuarios.push(usuario.id)
            }
        }
        return ids_usuarios
    }
    ...
    inserir_novo_post(post: Post, id: number): void {
        const aux: No<RedeSocial> = this.consultar(id)
    ...
    curtir_post(id_post: number, id_rede: number): void {
        try {
            const aux: No<RedeSocial> = this.consultar(id_rede)
...
class Dados_Aplicacao {
    ...
    get usuario_logado(): Usuario {
        if (this._usuario_logado == null) {
            throw new usuario_nao_logado("Erro, não há nenhum usuário logado!!!")
        }
    ...
    private atualizar_usuario(): void {
        if (this._usuario_logado == null) {
            throw new usuario_nao_logado("Erro, você não está logado!!!")
        }
